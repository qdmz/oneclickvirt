...
const resetLevelLimitsToDefault = () => {
  ElMessageBox.confirm(
    ...
    // 恢复为系统默认值
    addProviderForm.levelLimits = JSON.parse(JSON.stringify(systemLevelLimits.value))
...
// 从后端获取系统配置的等级限制
const fetchSystemLevelLimits = async () => {
  try {
    const res = await getSystemConfig()
    if (res.code === 200 && res.data?.quota?.levelLimits) {
      // 将后端返回的 kebab-case 转换为 camelCase
      const backendLevelLimits = res.data.quota.levelLimits
      const converted = {}
      for (const [level, limit] of Object.entries(backendLevelLimits)) {
        const levelNum = parseInt(level)
        converted[levelNum] = {
          maxInstances: limit['max-instances'] || limit.maxInstances || systemLevelLimits.value[levelNum].maxInstances,
          maxResources: {
            cpu: limit['max-resources']?.cpu || limit.maxResources?.cpu || systemLevelLimits.value[levelNum].maxResources.cpu,
            memory: limit['max-resources']?.memory || limit.maxResources?.memory || systemLevelLimits.value[levelNum].maxResources.memory,
            disk: limit['max-resources']?.disk || limit.maxResources?.disk || systemLevelLimits.value[levelNum].maxResources.disk,
            bandwidth: limit['max-resources']?.bandwidth || limit.maxResources?.bandwidth || systemLevelLimits.value[levelNum].maxResources.bandwidth
          },
          maxTraffic: limit['max-traffic'] || limit.maxTraffic || systemLevelLimits.value[levelNum].maxTraffic
        }
      }
      systemLevelLimits.value = converted
      console.log('系统等级限制已更新:', systemLevelLimits.value)
    }
  } catch (error) {
    console.error('获取系统等级限制失败:', error)
  }
}

// 解析等级限制配置（后端 kebab-case -> 前端 camelCase）
const parseLevelLimits = (levelLimitsStr) => {
  const defaultLevelLimits = systemLevelLimits.value
...
// 处理WebShell连接
const handleWebshell = async (provider) => {
  try {
    console.log('handleWebshell called with provider:', {
      id: provider.id,
      name: provider.name
    })
    // 使用sshStore创建SSH连接，true表示管理员模式，true表示这是一个节点服务器
    sshStore.createConnection(provider.id, provider.name, true, true)
    console.log('sshStore.connections after create:', sshStore.connections)
    ElMessage.success(t('admin.providers.webshellSuccess'))
  } catch (error) {
    ElMessage.error(t('admin.providers.webshellFailed') + ': ' + (error.message || t('common.unknownError')))
  }
}
