        ...
        <template #default="scope">
          <div class="action-buttons">
            ...
            <el-button
              size="small"
              type="primary"
              @click="handleAction('webshell', scope.row)"
            >
              {{ $t('admin.providers.webshell') }}
            </el-button>
            <el-button
              size="small"
              type="danger"
...
const handleAction = (action, row) => {
  // 如果传入了row参数，直接使用该row，否则使用currentRow.value
  ...
  switch (action) {
    case 'auto-configure':
      emit('auto-configure', targetRow)
      break
    case 'traffic-monitor':
      emit('traffic-monitor', targetRow)
      break
    case 'health-check':
      emit('health-check', targetRow.id)
      break
    case 'freeze':
      emit('freeze', targetRow.id)
      break
    case 'unfreeze':
      emit('unfreeze', targetRow)
      break
    case 'webshell':
      emit('webshell', targetRow)
      break
  }
