...
func InitAdminRouter(Router *gin.RouterGroup) {
	...
	AdminGroup.Use(middleware.RequireAuth(authModel.AuthLevelAdmin))
	{
		...
		// 节点管理
		AdminGroup.GET("/providers/:id/ssh", admin.AdminProviderSSHWebSocket) // 管理员WebSocket SSH连接到节点服务器
		...
		// 站点配置管理
		AdminGroup.GET("/site-configs", admin.GetSiteConfigs)
		AdminGroup.GET("/site-configs/:key", admin.GetSiteConfig)
		AdminGroup.PUT("/site-configs", admin.UpdateSiteConfigs)
		AdminGroup.POST("/site-configs/initialize", admin.InitializeSiteConfigs)

		// 产品管理
		AdminGroup.GET("/products", admin.GetProducts)
