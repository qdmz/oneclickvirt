...
func InitSeedData() {
	...
	initProducts() // 确保每次都执行产品初始化
	initOtherConfigurations()
	SeedSystemImages() // 确保系统镜像数据存在
...
// initProducts 初始化产品数据，确保每个等级只有一个产品
func initProducts() {
	global.APP_LOG.Info("开始初始化产品数据")
	...
	products := []product.Product{
		{
			...
			Disk:         1024,   // 1GB
			Bandwidth:    100,    // 100Mbps
			Traffic:      102400, // 100GB
			MaxInstances: 1,
			IsEnabled:    1,
			SortOrder:    1,
			Features:     `["1核CPU", "350MB内存", "1GB磁盘", "100Mbps带宽", "100GB流量"]`,
		},
		{
			Name:         "中级套餐",
			Description:  "适合小型团队和中型项目使用",
			Level:        2,
			Price:        19900, // 199元/月
			Period:       30,    // 30天
			CPU:          2,
			...
			Disk:         20480,  // 20GB
			Bandwidth:    200,    // 200Mbps
			Traffic:      204800, // 200GB
			MaxInstances: 3,
			IsEnabled:    1,
			SortOrder:    2,
			Features:     `["2核CPU", "1GB内存", "20GB磁盘", "200Mbps带宽", "200GB流量"]`,
		},
		{
			Name:         "高级套餐",
			Description:  "适合中型团队和大型项目使用",
			Level:        3,
			Price:        39900, // 399元/月
			Period:       30,    // 30天
			CPU:          4,
			...
			Disk:         40960,  // 40GB
			Bandwidth:    500,    // 500Mbps
			Traffic:      307200, // 300GB
			MaxInstances: 5,
			IsEnabled:    1,
			SortOrder:    3,
			Features:     `["4核CPU", "2GB内存", "40GB磁盘", "500Mbps带宽", "300GB流量"]`,
		},
		{
			Name:         "超级套餐",
			Description:  "适合大型团队和企业级项目使用",
			Level:        4,
			Price:        79900, // 799元/月
			Period:       30,    // 30天
			CPU:          8,
			...
			Disk:         81920,  // 80GB
			Bandwidth:    1000,   // 1000Mbps
			Traffic:      409600, // 400GB
			MaxInstances: 10,
			IsEnabled:    1,
			SortOrder:    4,
			Features:     `["8核CPU", "4GB内存", "80GB磁盘", "1000Mbps带宽", "400GB流量"]`,
		},
		{
			Name:         "旗舰套餐",
			Description:  "适合企业级应用和高并发场景",
			Level:        5,
			Price:        159900, // 1599元/月
			Period:       30,     // 30天
			CPU:          16,
			Memory:       8192,   // 8GB
			Disk:         163840, // 160GB
			Bandwidth:    2000,   // 2000Mbps
			Traffic:      512000, // 500GB
			MaxInstances: 20,
			IsEnabled:    1,
			SortOrder:    5,
			Features:     `["16核CPU", "8GB内存", "160GB磁盘", "2000Mbps带宽", "500GB流量"]`,
		},
	}

	// 获取当前所有产品
	var existingProducts []product.Product
	if err := global.APP_DB.Find(&existingProducts).Error; err != nil {
		global.APP_LOG.Error("获取现有产品失败", zap.Error(err))
		return
	}
	...
	for _, prod := range products {
		...
		result := global.APP_DB.Where("level = ?", prod.Level).First(&existingProduct)

		if result.Error == gorm.ErrRecordNotFound {
			// 产品不存在，创建新产品
			err := dbService.ExecuteTransaction(context.Background(), func(tx *gorm.DB) error {
				return tx.Create(&prod).Error
			})
			if err != nil {
				global.APP_LOG.Error("创建产品失败", zap.Error(err), zap.Int("level", prod.Level))
			}
			global.APP_LOG.Info("创建新产品", zap.String("name", prod.Name), zap.Int("level", prod.Level))
		} else if result.Error == nil {
			// 产品存在，更新产品
			err := dbService.ExecuteTransaction(context.Background(), func(tx *gorm.DB) error {
				return tx.Model(&existingProduct).Updates(prod).Error
			})
			if err != nil {
				global.APP_LOG.Error("更新产品失败", zap.Error(err), zap.Int("level", prod.Level))
			} else {
				global.APP_LOG.Info("更新产品", zap.String("name", prod.Name), zap.Int("level", prod.Level))
			}
		}
	}
	...
	// 删除多余的产品（每个等级只保留一个产品）
	// 按等级分组，找到每个等级的产品ID列表
	var levelProductIDs map[int][]uint
	levelProductIDs = make(map[int][]uint)

	if err := global.APP_DB.Model(&product.Product{}).Select("id, level").Find(&existingProducts).Error; err != nil {
		global.APP_LOG.Error("获取产品ID失败", zap.Error(err))
		return
	}

	for _, p := range existingProducts {
		levelProductIDs[p.Level] = append(levelProductIDs[p.Level], p.ID)
	}

	// 删除每个等级中除了第一个以外的所有产品
	for level, ids := range levelProductIDs {
		if len(ids) > 1 {
			// 保留第一个产品，删除其余产品
			deleteIDs := ids[1:]
			err := dbService.ExecuteTransaction(context.Background(), func(tx *gorm.DB) error {
				return tx.Delete(&product.Product{}, deleteIDs).Error
			})
			if err != nil {
				global.APP_LOG.Error("删除多余产品失败", zap.Error(err), zap.Int("level", level), zap.Uints("ids", deleteIDs))
			} else {
				global.APP_LOG.Info("删除多余产品", zap.Int("level", level), zap.Uints("ids", deleteIDs))
			}
		}
	}

	global.APP_LOG.Info("产品初始化完成")
}
...
func initOtherConfigurations() {
	...
	configs := []admin.SystemConfig{
		...
		},
		{
			Key:         "system_name",
			Value:       "虚拟化管理平台",
			Description: "系统名称，显示在登录页面和顶部导航栏",
			Category:    "system",
			Type:        "string",
			IsPublic:    true,
		},
		{
			...
			Value:       "支持多种虚拟化技术的管理平台",
			Description: "系统描述，显示在登录页面",
			Category:    "system",
			Type:        "string",
			IsPublic:    true,
		},
