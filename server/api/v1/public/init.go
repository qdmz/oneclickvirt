...
// GetPublicProducts 获取公开的产品列表
// @Summary 获取公开的产品列表
// @Description 获取可以公开访问的产品列表，用于首页展示
// @Tags 公开API
// @Accept json
// @Produce json
// @Success 200 {object} common.Response
// @Router /v1/public/products [get]
func GetPublicProducts(c *gin.Context) {
	var products []productModel.Product
	if err := global.APP_DB.Where("is_enabled = ?", 1).
		Order("sort_order ASC, id ASC").
		Find(&products).Error; err != nil {
		global.APP_LOG.Error("获取公开产品列表失败", zap.Error(err))
		c.JSON(http.StatusInternalServerError, common.Error("获取产品列表失败"))
		return
	}

	// 从用户等级限制中自动填充产品资源配置，确保产品配置与最新的等级配额一致
	for i := range products {
		// 直接实现与admin包中相同的逻辑，避免循环导入
		levelLimits := global.APP_CONFIG.Quota.LevelLimits
		level := products[i].Level
		
		if levelInfo, exists := levelLimits[level]; exists {
			// 填充最大实例数
			products[i].MaxInstances = levelInfo.MaxInstances
			
			// 填充最大流量（直接使用int64类型，无需转换）
			products[i].Traffic = levelInfo.MaxTraffic
