...
func (s *Service) prepareInstanceCreation(ctx context.Context, task *adminModel.Task) (*providerModel.Instance, error) {
	...
	err = dbService.ExecuteTransaction(ctx, func(tx *gorm.DB) error {
		...
		// 获取用户信息，使用用户的LevelExpireAt作为实例到期时间
		var user userModel.User
		if err := tx.First(&user, task.UserID).Error; err != nil {
			return fmt.Errorf("获取用户信息失败: %v", err)
		}

		// 设置实例到期时间，优先使用用户的LevelExpireAt
		var expiredAt time.Time
		if user.LevelExpireAt != nil {
			// 使用用户的等级到期时间
			expiredAt = *user.LevelExpireAt
		} else if provider.ExpiresAt != nil {
			// 如果用户没有到期时间，使用Provider的到期时间
