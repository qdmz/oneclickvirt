...
func (s *Service) ResetPasswordAndNotify(userID uint) (string, error) {
	global.APP_LOG.Info("开始密码重置流程", zap.Uint("user_id", userID))
	
	var user userModel.User
	if err := global.APP_DB.First(&user, userID).Error; err != nil {
		global.APP_LOG.Error("查询用户失败", zap.Uint("user_id", userID), zap.Error(err))
		return "", errors.New("用户不存在")
	}
	
	global.APP_LOG.Info("找到用户", zap.Uint("user_id", userID), zap.String("username", user.Username))

	// 生成强密码（12位）
	newPassword := utils.GenerateStrongPassword(12)
	global.APP_LOG.Info("生成新密码", zap.Uint("user_id", userID), zap.String("password", newPassword))

	// 密码强度验证（确保生成的密码符合策略）
	if err := utils.ValidatePasswordStrength(newPassword, utils.DefaultPasswordPolicy, user.Username); err != nil {
		global.APP_LOG.Error("密码强度验证失败", zap.Uint("user_id", userID), zap.Error(err))
		return "", err
	}
	
	global.APP_LOG.Info("密码强度验证通过", zap.Uint("user_id", userID))

	// 加密新密码
	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(newPassword), bcrypt.DefaultCost)
	if err != nil {
		global.APP_LOG.Error("密码加密失败", zap.Uint("user_id", userID), zap.Error(err))
		return "", err
	}
	
	global.APP_LOG.Info("密码加密成功", zap.Uint("user_id", userID))

	// 更新密码
	if err := global.APP_DB.Model(&user).Update("password", string(hashedPassword)).Error; err != nil {
		global.APP_LOG.Error("密码更新失败", zap.Uint("user_id", userID), zap.Error(err))
		return "", err
	}
	
	global.APP_LOG.Info("密码更新成功", zap.Uint("user_id", userID))
	...
	global.APP_LOG.Info("密码重置流程完成", zap.Uint("user_id", userID))
	return newPassword, nil
}
...
func (s *Service) sendPasswordToUser(user *userModel.User, newPassword string) error {
	...
	// 用户未绑定任何通信渠道，记录日志但不返回错误
	global.APP_LOG.Info("用户未绑定任何通信渠道，密码已重置但无法发送通知",
