...
func syncConfigToGlobal(key string, oldValue, newValue interface{}) error {
	...
	case "payment":
		if paymentConfig, ok := newValue.(map[string]interface{}); ok {
			syncPaymentConfig(paymentConfig)
		}
	}
	return nil
}
...
// syncPaymentConfig 同步支付配置 - 只支持 kebab-case 格式
func syncPaymentConfig(paymentConfig map[string]interface{}) {
	// 支付宝配置
	if v, ok := paymentConfig["alipay-app-id"].(string); ok {
		global.APP_CONFIG.Payment.AlipayAppID = v
	}
	if v, ok := paymentConfig["alipay-gateway"].(string); ok {
		global.APP_CONFIG.Payment.AlipayGateway = v
	}
	if v, ok := paymentConfig["alipay-private-key"].(string); ok {
		global.APP_CONFIG.Payment.AlipayPrivateKey = v
	}
	if v, ok := paymentConfig["alipay-public-key"].(string); ok {
		global.APP_CONFIG.Payment.AlipayPublicKey = v
	}
	if v, ok := paymentConfig["enable-alipay"].(bool); ok {
		global.APP_CONFIG.Payment.EnableAlipay = v
	}

	// 微信支付配置
	if v, ok := paymentConfig["wechat-app-id"].(string); ok {
		global.APP_CONFIG.Payment.WechatAppID = v
	}
	if v, ok := paymentConfig["wechat-mch-id"].(string); ok {
		global.APP_CONFIG.Payment.WechatMchID = v
	}
	if v, ok := paymentConfig["wechat-api-key"].(string); ok {
		global.APP_CONFIG.Payment.WechatAPIKey = v
	}
	if v, ok := paymentConfig["wechat-api-v3-key"].(string); ok {
		global.APP_CONFIG.Payment.WechatAPIV3Key = v
	}
	if v, ok := paymentConfig["wechat-serial-no"].(string); ok {
		global.APP_CONFIG.Payment.WechatSerialNo = v
	}
	if v, ok := paymentConfig["wechat-type"].(string); ok {
		global.APP_CONFIG.Payment.WechatType = v
	}
	if v, ok := paymentConfig["enable-wechat"].(bool); ok {
		global.APP_CONFIG.Payment.EnableWechat = v
	}

	// 余额支付配置
	if v, ok := paymentConfig["balance-daily-limit"].(float64); ok {
		global.APP_CONFIG.Payment.BalanceDailyLimit = int(v)
	} else if v, ok := paymentConfig["balance-daily-limit"].(int); ok {
		global.APP_CONFIG.Payment.BalanceDailyLimit = v
	}
	if v, ok := paymentConfig["balance-min-amount"].(float64); ok {
		global.APP_CONFIG.Payment.BalanceMinAmount = int(v)
	} else if v, ok := paymentConfig["balance-min-amount"].(int); ok {
		global.APP_CONFIG.Payment.BalanceMinAmount = v
	}
